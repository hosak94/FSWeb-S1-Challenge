...
function indekseGoreFenomen(fenomenler,indeks) {
  const profile =fenomenler[indeks].profile;
...
const yeniFenomenler = fenomenEkle(fenomenler, 6, "Workintech", 10000000, 2022, "Instagram");
...
function enFenomenler(fenomenler) {
  const enFenomenlerArr = [];

  for (let i = 0; i < fenomenler.length; i++) {
    if (fenomenler[i].followers > 100000000) {
      enFenomenlerArr.push(fenomenler[i].profile);
    }
  }

  return enFenomenlerArr;
}
...
function fenomenGonderimSayisi(fenomenler, profile) {
  const index = fenomenler.findIndex(fenomen => fenomen.profile === profile);
  if (index === -1) {
    return -1;
  }
...
function platformaGoreCokGonderiYapanFenomen(fenomenler, platform) {
  ...
  for (let i = 0; i < fenomenler.length; i++) {
    ...
    if (fenomen.platform === platform && fenomen.posts !== 'NA' && fenomen.posts > maxGonderi) {
      maxGonderi = fenomen.posts;
      maxFenomen = fenomen.name;
    }
  }
